...
- (NSURL *)URLForStoreFromPackage:(NSString *)package
{
    ...
    if (package) {
        for (NSString *extension in @[ @"sqlite", @"sqlite-shm", @"sqlite-wal"]) {
            NSString *packageDirectory = [@"Data" stringByAppendingPathComponent:package];
            NSString *sqliteFilePath = [[NSBundle bundleForClass:self.class] pathForResource:kStoreName ofType:extension inDirectory:packageDirectory];
